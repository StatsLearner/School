...
The second plot shows you the histograms of the number of samples iterated in which the proportion of patients who are in each stratum level in total tretment 1 allocation. The proportion of treatment 1 varies a lot and does not look at normal distributin at all. The others have curve-like distributions, yet not like a normal distribution.

The third plot shows you mean proportions in each iteration. As the number of iteration increases, the means should approach to the theoretical means. However, 50 time iterations might not be sufficient to see the expected convergence.The precisions of B1 and C1 are higher than those of the others.

One note is that these results are likely to be influenced by the initial imbalance in each stratum. Particularly the StratumA is imbalanced in the first place.
...
The plots are not converged at all. The histograms clearly show uniform distributions. This may be due to the method of modifying probability on treatment allocation. Modifying the probability may need to be done more strongly, e.g., $P(Treatment 1)= 0.75$ and $P(Treatment 2) = 0.25$ if treatment 1 is insufficient.
...
## Conclusion
Minimisation process can produce imbalance treatment allocation, depending on how to modify/update the probability of allocating intervention.
